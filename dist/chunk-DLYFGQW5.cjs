...
var scanForServerFiles = async (config, devServer) => {
  ...
  if (!server) {
    ...
    server = await createServer({
      server: { ..._optionalChain([config, 'optionalAccess', _ => _.server]) || {}, middlewareMode: true },
      appType: "custom",
      base: config.base
    });
  }
...
function getCookies(req) {
  const cookieHeader = !isExpressRequest(req) ? req.headers.cookie : _optionalChain([req, 'access', _2 => _2.get, 'optionalCall', _3 => _3("cookie")]);
...
var createCSRF = (initialOptions = {}) => {
  ...
  return (req, res, next) => {
    ...
    _optionalChain([next, 'optionalCall', _4 => _4()]);
  };
};
...
var createMiddleware = (initialOptions = {}) => {
  ...
  return async (req, res, next) => {
    ...
    try {
      ...
      if (rpcPrefix && !_optionalChain([url, 'optionalAccess', _6 => _6.startsWith, 'call', _7 => _7(rpcPrefix)])) {
        return _optionalChain([next, 'optionalCall', _8 => _8()]);
      }
      if (!handler) return _optionalChain([next, 'optionalCall', _9 => _9()]);
      if (headers) {
...
var createRPCMiddleware = (initialOptions = {}) => {
  ...
  return createMiddleware({
    ...
    handler: async (req, res, next) => {
      const url = isExpressRequest(req) ? req.originalUrl : req.url;
      if (!_optionalChain([url, 'optionalAccess', _11 => _11.startsWith, 'call', _12 => _12(`/${options.rpcPrefix}/`)])) {
